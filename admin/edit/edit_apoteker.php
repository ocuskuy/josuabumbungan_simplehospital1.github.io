...
$query = "SELECT * FROM data_apoteker ORDER BY id_apoteker ASC";
$result = mysqli_query($conn, $query);

// Memeriksa apakah tombol hapus di klik
if(isset($_POST["hapus"])) {
  // Mengambil data id yang akan dihapus
  $id = $_POST["id_apoteker"];

  // Query untuk menghapus data dari database
  $delete_query = "DELETE FROM data_apoteker WHERE id_apoteker='$id'";
...
<html lang="en">
  <head>
    ...
    <title>Halaman daftar apoteker</title>
  ...
  <body>
    ...
    <div class="p-5 bg-body-tertiary rounded-3">
        <h4 class="text-body-emphasis text-center">Daftar apoteker</h4>
        <!-- content -->
        <table class="table table-bordered">
            <thead>
                <tr>
                    ...
                    <th scope="col">riwayat apoteker</th>
                    <th scope="col">alamat apoteker</th>
            ...
            <tbody>
                      ...
                      <?php while($row = mysqli_fetch_assoc($result)) : ?>
                        <tr>
                          ...
                          <td><?= $row["riwayat_apoteker"];?></td>
                          <td><?= $row["alamat_apoteker"];?></td>
                          <td><?= $row["notelp_apoteker"];?></td>
                          <td><?= $row["password_apoteker"];?></td>
                          <td>
                              <form method="post">
                                  <input type="hidden" name="id_apoteker" value="<?= $row["id_apoteker"]; ?>">
                                  <button type="submit" name="hapus">Hapus</button>
                                  <?php echo "<td><a href='edit2_apoteker.php?id_apoteker=".$row['id_apoteker']."'>Edit</a></td>"; ?>
                              </form>
                          </td>
                        </tr>
            </tbody>
